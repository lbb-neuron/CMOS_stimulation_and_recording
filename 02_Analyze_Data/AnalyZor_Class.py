# Class to import and analyze files recorded with the MaxOne system
...
class AnalyZor:
    def __init__(
        ...
        if dateIndex == -1:
            if yearIndex:
                dateIndex = int(self.yearIndex*1e4 + self.monthIndex*1e2 + self.dayIndex)
            else:
                dateIndex = 20220101
        ...
        self.stimulationFrequenciesList = stimulation_frequency
        self.stimulationVoltagesList = stimulation_voltages
        self.spike_mat = None

        self.auto_parse_legacy = auto_parse_legacy
        self.auto_parse = auto_parse

        if auto_parse_legacy:
            i = 0
            for j in range(indexRange):
                string = ""
                while filename[i] != "_" and filename[i] != ".":
                    string += str(filename[i])
                    i += 1
                i += 1
                self.filenameList.append(string)
            del i
            self.outputFolder = os.path.join(
                outputPath,
                self.filenameList[yearIndex]
                + "_"
                + self.filenameList[monthIndex]
                + "_"
                + self.filenameList[dayIndex],
                self.filenameList[chipIDIndex],
                self.filenameList[DIVIndex],
                "N" + self.filenameList[networkNrIndex],
                self.filenameList[customTextIndex],
            )
            try:
            ...
            if (stimulation_electrodes != [0]) and (stimulation_frequency is None):
                self.stimulationFrequenciesList = [4]
    ...
    def loadDataSpikesOnly(
        ...
        blankingThresh=50,
        blankingWindow=[-10, 10],
        return_spike_mat = False, 
        store_relative_timing = True, 
    ):
        """
        ...
        if return_spike_mat:
            ...
            channelMappingDense = np.zeros(int(np.max(self.electrodeChannelMapping[1, :]+1)))
            for ch_id, ch in enumerate(self.electrodeChannelMapping[1, :]):
                channelMappingDense[ch] = self.electrodeChannelMapping[0, ch_id]+1 # loosing electrode 0
            
            routed_indices = channelMappingDense[channels].astype(bool)
            channelMappingDense = channelMappingDense-1
            spikeTimes = spikeTimes[routed_indices]
            amplitudes = amplitudes[routed_indices]

            self.spike_mat = np.zeros((len(spikeTimes),4), dtype=float)
            self.spike_mat[:,0] = channelMappingDense[channels[routed_indices]]

            if store_relative_timing and len(self.stimulatedElectrodesList):
                spike_relative = spikeTimes - spikeTimesArtefact[:,None]
                spike_relative_limited = spike_relative
                spike_relative_limited[spike_relative<0] = self.sampleFrequency
                stim_iteration = np.squeeze(np.argmin(spike_relative_limited, axis=1))

                # before first stimulus use original if there already was a spike
                if spike_relative_limited[0,0] == self.sampleFrequency:
                    spike_relative_limited[0,:] = spikeTimes
                # print(f"Stim iteration {stim_iteration.shape} spike rel {spike_relative.shape} artefact {spikeTimesArtefact.shape} and {spikeTimesArtefact[:5]}")

                for it_id, it in enumerate(stim_iteration[:-1]):
                    self.spike_mat[it:stim_iteration[it_id+1],1] = spike_relative[it_id,it:stim_iteration[it_id+1]]/self.sampleFrequency*1e3
                    self.spike_mat[it:stim_iteration[it_id+1],3] = it_id
                self.spike_mat[stim_iteration[it_id+1]:,1] = spike_relative[it_id,stim_iteration[it_id+1]:]/self.sampleFrequency*1e3
                self.spike_mat[stim_iteration[it_id+1]:,3] = it_id+1
            else:
                self.spike_mat[:,1] = spikeTimes/self.sampleFrequency*1e3
            self.spike_mat[:,2] = amplitudes
    ...
    def export_to_dict(
        ...
        ''' 
        export to dictionary with following keys
            ID:     unique chip id
            NW:     network ID starting with 0 on top left corner 
            DATE:   recording date in style 'YYYYMMDD'
            DIV:    days in vitro of culture at recording date
            STIMEL: stimulation electrode, None if spontaneous data
            STIMV:  stimulation voltage
            STIMF:  stimulation frequency 

            SPIKEMAT:   numpy array of shape num_of_spikes x 4 [[#EL, t, amp, #iteration]]
                #EL:    electrode number
                t:      timing with respect to start (spontaneous)
                amp:    spike amplitude
                #iteration:  stimulus iteration, all zeros for spontaneous  
            COLOUR:     colour code matrix
        '''
        if IDcustom is not None:
            self.chipIDIndex = IDcustom
        if NWcustom is not None:
            self.networkNrIndex = NWcustom        
        if DATEcustom is not None:
            self.dateIndex = DATEcustom
        else:
            self.dateIndex = self.dateIndex 
        if DIVcustom is not None:
            self.DIVIndex = DIVcustom
        if STIMELcustom is not None:
            if type(STIMELcustom) == int:
                self.stimulatedElectrodesList = [STIMELcustom]
            else:
                self.stimulatedElectrodesList = STIMELcustom
        if STIMVcustom is not None:
            if type(STIMELcustom) == int:
                self.stimulationVoltagesList = [STIMVcustom]
            else:
                self.stimulationVoltagesList = STIMVcustom
        if STIMFcustom is not None:
            self.stimulationFrequency = STIMFcustom

        if self.spike_mat is None:
            spike_list = [] 
            for s_per_el in self.spikes:
                for s_id, s in enumerate(s_per_el[0]):
                    spike_list.append(np.array([s_per_el[1]['electrode'], s/self.sampleFrequency*1e3, s_per_el[1]['peak_heights'][s_id], 0]))

            self.spike_mat = np.array(spike_list, dtype=float)
        # print(f"Spike matrix of shape {self.spike_mat.shape}")

        # load analyzor entries to network dict
        network_dict = {
            'DATE':     self.dateIndex, 
            'ID':       self.chipIDIndex, 
            'NW':       self.networkNrIndex, 
            'DIV':      self.DIVIndex, 
            'STIMEL':   self.stimulatedElectrodesList, 
            'STIMV':    self.stimulationVoltagesList, 
            'STIMF':    self.stimulationFrequenciesList, 
        ...
        if exportFilename is None:
            ...
            for k in network_dict.keys():
                if network_dict[k] is not None:
                    if (
                            (k=='STIMV' or k=='STIMEL') and (network_dict[k]==[0] or network_dict[k]==[])
                            or k=='SPIKEMAT'):
                        continue
                    if type(network_dict[k]) == list:
                        exportFilename += f'{k}{network_dict[k][0]}_'
                    else:
                        exportFilename += f'{k}{network_dict[k]}_'

        if exportFolder is not None:
            exportFilename = f"{exportFolder}/{exportFilename}"
        with open(f"{exportFilename}.pkl","wb") as f:
